...
export default function Dashboard() {
  ...
  // Move loadNews outside useEffect so it can be accessed by handleScroll
      ...
      try {
        ...
        if (res?.data?.status) {
          const mapped = res.data.data.map((d) => ({
            name: d.portal_name,
            posts: d.total_distributions,
            traffic: `${d.successful_distributions} success / ${d.failed_distributions} failed`,
            status: d.failed_distributions > 0 ? "Partial" : "Active",
          }));
          setDomains(mapped);
        }
      } catch (err) {
...
//       // For page 1 replace, for others append
//       setRecentPosts((prev) =>
//         pageNumber === 1 ? mapped : [...prev, ...mapped]
//       );
//       setPagination(res.data.pagination);
...
//             </div>
//             <div className="p-6">
//              <div className="space-y-4 overflow-y-auto max-h-96 recent-posts-scroll">
//           {recentPosts.map((post) => (
//             <div key={post.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
//                     <div className="flex-1">
//                       <h4 className="font-medium text-gray-900 mb-1">
