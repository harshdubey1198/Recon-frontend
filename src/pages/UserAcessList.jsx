  ...
  export default function UserAccessTable() {
    ...
    // Load users from API with optional search
    const loadUsers = async (page = 1, search = "") => {
      if (loadingPage) return;
      
      setLoadingPage(true);
      try {
        const res = await fetchUserDetails(page, search);
        ...
        if (apiResponse.status && Array.isArray(apiResponse.data)) {
          ...
          // Replace or append users based on page number
    ...
    const handleSearch = (query) => {
      ...
      setUserAccess({}); // Clear existing access data
      loadUsers(1, query);
    };
          ...
          >
                                ...
                                <div className="flex items-start justify-between mb-4">
                                  <div className="flex items-center gap-3 flex-1">
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <p className="text-lg font-bold text-gray-900 hover:text-gray-700 hover:underline block truncate">
                                        {access.domain.name}
                                      </p>
                                    </div>
                                  </div>
            ...
            {loadingPage && currentPage > 1 && (
              <div className="text-center py-4">
                ...
                <p className="text-gray-500 text-sm mt-2">Loading more users...</p>
              </div>
            )}
            {!loadingPage && hasNext && (
