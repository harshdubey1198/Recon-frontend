...
const DraftsModal = ({ showDrafts, setShowDrafts, drafts, handleSelectDraft }) => {
  if (!showDrafts) return null;

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 relative">
        <button onClick={() => setShowDrafts(false)} className="absolute top-3 right-3 text-gray-400 hover:text-gray-700">
          ✕
        </button>
          ...
          {drafts.length > 0 ? (
            drafts.map((draft) => (
              ...
              >
                ...
                <p className="text-sm text-gray-600 font-semibold">Status: {draft.status}</p>
              </div>
            ))
          ) : (
