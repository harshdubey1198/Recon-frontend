...
const GA4Leaderboard = ({ range, customRange }) => {
  ...
  return (
    <div className="mb-8">
      {/* Header */}
        <div className="bg-black rounded-2xl p-4 sm:p-6 mb-5 flex items-center justify-between shadow-lg border border-gray-800">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-gray-300 text-sm sm:text-base">
                Aggregated metrics across {ga4PidOptions.length} GA4 properties
              </p>
            </div>
          </div>
          ...
          <button
            onClick={loadGA4}
            disabled={isLoading}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm bg-white/10 text-white border border-white/20 backdrop-blur-sm hover:bg-white/20 disabled:opacity-50 transition"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        />
          ...
          {/* Footer small info */}
          <div className="bg-gradient-to-r from-gray-50 to-gray-100 px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-200 mt-4 rounded-b-2xl">
      <p className="text-xs sm:text-sm text-gray-600 flex items-center justify-between w-full">
        <span>
          ⚡ GA4 data merged from DXB, CNI, GCC & other properties.
        </span>

        {lastUpdated && (
          <span className="text-gray-700 font-medium">Last updated at {lastUpdated}</span>
        )}
      </p>
    </div>
